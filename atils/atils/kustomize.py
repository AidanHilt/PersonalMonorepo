...
from atils import template_utils as template
...
def main(args: str):
  ...
  init_parser = subparsers.add_parser(
    "init", help="Initialize a new Kustomize template, with pre-populated environments"
  )
...
def add_resources_to_kustomization_files(directory: str, file_list: dict[str, str]):
  ...
  kustomization = yaml_utils.convert_yaml_to_dict(os.path.join(base_dir, "kustomization.yaml"))
  ...
  for subdir in os.listdir(overlays_dir):
    kustomization = yaml_utils.convert_yaml_to_dict(os.path.join(overlays_dir, subdir, "kustomization.yaml"))
    kustomization['patchesStrategicMerge'] = list(file_list.keys())

    with open(os.path.join(overlays_dir, subdir, "kustomization.yaml"), 'w') as file:
      yaml.dump(kustomization, file)
...
def initialize_kustomize_template():
  ...
  for subdir in os.listdir(overlays_dir):
    subdir_path = os.path.join(overlays_dir, subdir)
    if os.path.isdir(subdir_path):
      template.template_file('../templates/kustomization-overlay.yaml', os.path.join(overlays_dir, subdir, 'kustomization.yaml'))
