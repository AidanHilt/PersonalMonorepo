let
  hyperion = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIImw5CsGmsR1WTunv5bvNcozmoUSgJf76RMvy6SZtA2R aidan@hyperion";
  # We don't use secrets, but we need a second machine that can run agenix to handle setup
  workvm = "ssh-rsa  root@Aidans-Macbook-Pro";
  nixos-wsl = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHh4tIY/al9pyApJa+vH83V0okAvvSTmLuKiljt+v5wP nixos@wsl-machine";
in
{
  "smb-mount-config.age".publicKeys = [ hyperion nixos-wsl workvm ];
  "rclone-config.age".publicKeys = [ hyperion nixos-wsl workvm ];
  "kubeconfig.age".publicKeys = [ hyperion nixos-wsl workvm ];
}
